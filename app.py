...
# ── FractalShield constants ───────────────────────────────────────────────────
MAGIC_V4       = b"MFSUv4"
VERSION_V4     = b"\x04"
REAL_MAGIC     = b"MFSU\x04"
ORDER_SALT_FS  = b"MFSU_ORDER_SALT_"
...
SHIELD_NAMES   = {
    1: ("🛡️ Estándar",  "Documentos personales, fotos, notas",        "~0.5s",  "3.5×"),
    2: ("🔒 Reforzado", "Contratos, datos financieros, credenciales",  "~0.7s",  "7.5×"),
    3: ("💎 Máximo",    "Secretos críticos, datos médicos, legales",   "~1.3s",  "15.5×"),
}
...
# ══════════════════════════════════════════════════════════════════════════════
#  FRACTALSHIELD — DEFENSA ADAPTATIVA POR CAPAS
# ══════════════════════════════════════════════════════════════════════════════

def _fs_enc_block(data: bytes, password: str, salt: bytes, iv: bytes, kdf_m: int) -> bytes:
    """Cifra un bloque con KDF fractal de costo variable."""
    km = mfsu_kdf(password, salt, key_len=96)
    ks = mfsu_keystream(km[:64], iv, len(data))
    return (np.frombuffer(data, dtype=np.uint8) ^ ks).tobytes()


def _fs_pad(d: bytes, b: int = 16) -> bytes:
    pl = b - (len(d) % b)
    return d + bytes([pl] * pl)


def _fs_unpad(d: bytes) -> bytes:
    pl = d[-1]
    if pl < 1 or pl > 16 or d[-pl:] != bytes([pl] * pl):
        raise ValueError("Padding inválido")
    return d[:-pl]
...
def fractalshield_encrypt(plaintext: bytes, password: str, level: int = 2) -> bytes:
    """
    Cifra con FractalShield nivel 1/2/3.

    Genera N capas de cifrado del mismo tamaño:
    - Capa 0 (real):   REAL_MAGIC + plaintext
    - Capas 1..N-1:    datos fractal estadísticamente indistinguibles

    El orden está cifrado con la clave real — sin ella el atacante
    no sabe en qué posición está la capa real ni si acertó la contraseña.
    Cada capa señuelo usa KDF_M mayor → costo geométrico para el atacante.
    """
    kdf_ms = SHIELD_LEVELS[level]
    n      = len(kdf_ms)

    padded_real = _fs_pad(REAL_MAGIC + plaintext)
    L = len(padded_real)
    ...
    for i, kdf_m in enumerate(kdf_ms):
        ...
        layers.append(_fs_enc_block(data, password, salts[i], ivs[i], kdf_m))

    # Orden aleatorio usando el campo fractal
    ...
    order_enc = _fs_enc_block(_fs_pad(bytes(order)), password, ORDER_SALT_FS, iv_ord, 256)
    ...
    km_g = mfsu_kdf(password, salt_g, key_len=96)
...
def fractalshield_decrypt(blob: bytes, password: str) -> bytes:
    """
    Descifra un blob FractalShield v4.
    Verifica MAC global antes de descifrar — Encrypt-then-MAC.
    """
    if len(blob) < 12 or not blob.startswith(MAGIC_V4):
    ...
    level = blob[7]; n = blob[8]
    if level not in SHIELD_LEVELS:
        raise ValueError("Nivel de shield inválido")
    kdf_ms = SHIELD_LEVELS[level]
    ...
    if not hmac_mod.compare_digest(mac_s, mac_c):
        raise ValueError("Autenticación fallida — contraseña incorrecta o archivo alterado")

    order_plain = _fs_enc_block(order_enc, password, ORDER_SALT_FS, iv_ord, 256)
    order = list(_fs_unpad(order_plain))
...
def fractalshield_inspect(blob: bytes) -> dict:
    ...
    return {
        ...
        "shield_level": level, "shield_name": name,
        "n_layers": n, "mac": mac.hex(),
        "layer_size": L_layer, "total_size": len(blob),
...
def main():
    ...
    # ── Header ───────────────────────────────────────────────────────────────
    st.markdown('<div class="main-title">🌀 Fractalyx</div>',
                unsafe_allow_html=True)
    ...
    # ══ TAB 1: CIFRADO ═══════════════════════════════════════════════════════
    with tab1:
        st.subheader("Cifrado MFSU — Elige tu nivel de protección")

        # ── Selector de nivel FractalShield ──────────────────────────────────
        st.markdown("#### 🌀 FractalShield — Capas defensivas fractales")
        st.markdown(
            "El archivo cifrado contiene **N capas independientes**. "
            "Solo la clave correcta revela cuál es la real. "
            "El atacante paga el costo de **todas** las capas sin saber si acertó."
        )

        col_l1, col_l2, col_l3 = st.columns(3)
        level_selected = st.session_state.get("shield_level", 2)

        for col, lvl in zip([col_l1, col_l2, col_l3], [1, 2, 3]):
            name, use, t_user, cost = SHIELD_NAMES[lvl]
            n_layers = len(SHIELD_LEVELS[lvl])
            is_sel = (lvl == level_selected)
            border_color = "#00c8ff" if is_sel else "#1a1a2e"
            bg_color     = "#0d1a2a" if is_sel else "#08080e"
            with col:
                st.markdown(f"""
                <div style="border:2px solid {border_color};border-radius:10px;
                            padding:1rem;background:{bg_color};text-align:center;
                            cursor:pointer;transition:all 0.2s;">
                    <div style="font-size:1.4rem">{name}</div>
                    <div style="color:#aaa;font-size:0.78rem;margin:0.4rem 0">{use}</div>
                    <div style="color:#00c8ff;font-size:0.82rem">
                        {n_layers} capas &nbsp;·&nbsp; usuario: {t_user}
                    </div>
                    <div style="color:#ff6b35;font-size:0.78rem;margin-top:0.3rem">
                        atacante: {cost} más costoso
                    </div>
                </div>
                """, unsafe_allow_html=True)
                if st.button(
                    f"Seleccionar {'✓' if is_sel else ''}",
                    key=f"sel_{lvl}",
                    use_container_width=True,
                    type="primary" if is_sel else "secondary"
                ):
                    st.session_state["shield_level"] = lvl
                    st.rerun()

        level_selected = st.session_state.get("shield_level", 2)
        sel_name = SHIELD_NAMES[level_selected][0]
        st.info(f"Nivel activo: **{sel_name}** — {len(SHIELD_LEVELS[level_selected])} capas fractal")

        st.markdown("---")

        # ── Modo de cifrado: Standard v3 o FractalShield ─────────────────────
        modo = st.radio(
            "Modo de cifrado",
            ["🌀 FractalShield (recomendado)", "⚡ Estándar v3 (más rápido)"],
            horizontal=True,
            key="modo_cifrado"
        )
        use_shield = "FractalShield" in modo

        c1, c2 = st.columns(2)

        with c1:
            st.markdown("#### 🔐 Cifrar")
            f_enc = st.file_uploader("Archivo a cifrar", key="enc")
            p_enc = st.text_input("Contraseña", type="password", key="pe")

            if st.button("Cifrar", use_container_width=True, type="primary"):
                if not f_enc or not p_enc:
                    st.warning("Necesitas archivo y contraseña.")
                else:
                    spinner_msg = (
                        f"FractalShield nivel {level_selected} — generando {len(SHIELD_LEVELS[level_selected])} capas fractales..."
                        if use_shield else
                        "KDF Memory-Hard + evolución ψ..."
                    )
                    with st.spinner(spinner_msg):
                        ...
                        try:
                            ...
                            m3.metric("Modo",      "🌀 Shield" if use_shield else "⚡ v3")
                            m4.metric("Nivel",     sel_name if use_shield else "v3")

                            st.download_button(
                                f"⬇️ Descargar {f_enc.name + ext}",
                                data=blob,
                                file_name=f_enc.name + ext,
        ...
        with c2:
            ...
            if st.button("Descifrar", use_container_width=True):
                if not f_dec or not p_dec:
                    st.warning("Necesitas el archivo y la contraseña.")
                else:
                    with st.spinner("Verificando capas + reconstruyendo ψ..."):
                        blob = f_dec.read()
                        try:
                            t0 = time.time()
                            # Detectar formato automáticamente
                            if blob.startswith(MAGIC_V4):
                                pt = fractalshield_decrypt(blob, p_dec)
                                modo_dec = "🌀 FractalShield"
                            else:
            ...
            if blob_i[:6] == MAGIC_V4:
                ...
                if info["valid"]:
                    ...
                    with ia:
                        ...
                        st.code(
                            f"Formato:     MFSUv4 (FractalShield)\n"
                            f"Nivel:       {info['shield_level']} — {info['shield_name']}\n"
                            f"Capas:       {info['n_layers']}\n"
                            f"KDF_M/capa:  {info['kdf_ms']}\n"
                            f"MAC:         {info['mac'][:32]}...",
                            language=None
                        )
                    with ib:
                        st.metric("Nivel shield",   info["shield_name"])
                        st.metric("Capas",          info["n_layers"])
                        st.metric("Tamaño/capa",    f"{info['layer_size']:,} B")
                        st.metric("Costo atacante", SHIELD_NAMES.get(info["shield_level"],("?","?","?","?"))[3])
                        st.caption("Todas las capas tienen el mismo tamaño — el atacante no puede identificar la real.")
